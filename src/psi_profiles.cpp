...
#ifdef YAPSI
	opt_log_on_server = o->getOption(base + ".log-on-server").toBool();
	jabber_service = o->getOption(base + ".jabber-service").toString();
#endif
...
#ifdef YAPSI
	o->setOption(base + ".log-on-server", opt_log_on_server);
	o->setOption(base + ".jabber-service", jabber_service);
#endif
...
#ifdef YAPSI
	readBoolAttribute(a, "log-on-server", &opt_log_on_server);
...
#ifdef YAPSI
				migrateStringEntry(tag, "subscribe", "options.ui.notifications.sounds.system-subscribe");
				migrateStringEntry(tag, "unsubscribe", "options.ui.notifications.sounds.system-unsubscribe");
#endif
			}
		}
