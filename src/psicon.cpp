...
public slots:
	void saveOpenedChats()
	{
		...
		foreach(TabDlg* tabDlg, tabManager->tabSets()) {
			for (int j = 0; j < tabDlg->tabCount(); ++j) {
...
#ifdef YAPSI
static void setupFontSubstitutions()
{
	// also check yapsi.qss and default.xml
...
bool PsiCon::init()
{
	...
	PsiOptions::instance()->setOption("options.service-discovery.enable-entity-capabilities", true);
	PsiOptions::instance()->setOption("options.ui.notifications.successful-subscription", true);
	PsiOptions::instance()->setOption("options.vcard.query-own-vcard-on-login", true);
	PsiOptions::instance()->setOption("options.messages.force-incoming-message-type", "chat");
