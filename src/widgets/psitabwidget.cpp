...
void PsiTabWidget::addTab(QWidget *widget, QString name) {
	insertTab(count(), widget, name);
}

int PsiTabWidget::insertTab(int index, QWidget* widget, const QString& name)
{
	Q_ASSERT(widget);
	...
	stacked_->insertWidget(index, widget);
...
void PsiTabWidget::closeButtonClicked()
{
	emit closeTab(currentPageIndex());
}

void PsiTabWidget::doBlockSignals(bool blockSignals)
{
	this->blockSignals(blockSignals);
	this->tabBar_->blockSignals(blockSignals);
}
